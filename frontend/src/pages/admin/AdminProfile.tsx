...
import {
  ...
  AlertCircle, Loader2, Hash, Key, Database, Settings,
} from "lucide-react";
...
const Section = ({ icon: Icon, title, accent = "text-blue-400", aside, children }: {
...
export default function AdminProfile() {
  const { isAuthenticated, isAdmin, setUser } = useAuth() as {
    isAuthenticated: boolean; isAdmin: boolean; setUser?: (u: User) => void;
  };
    ...
    <DashboardLayout userType="admin">
      <div className="w-full max-w-2xl px-6 py-8 space-y-8">
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            <span className="text-xs font-semibold text-slate-500 tracking-widest uppercase">Admin · Account</span>
            <h1 className="text-3xl font-bold text-white tracking-tight">Profile Settings</h1>
            <p className="text-slate-400 mt-1 text-sm">Manage your administrator profile</p>
          </div>
        </div>

        {/* ── Identity card — blue/indigo for admin ── */}
        <div className="rounded-2xl border border-blue-500/20 bg-blue-500/[0.04] backdrop-blur-sm p-6">
          <div className="flex items-center gap-5">
            <div className="w-20 h-20 rounded-2xl flex-shrink-0 flex items-center justify-center text-2xl font-bold text-white tracking-tight select-none bg-gradient-to-br from-blue-600 to-indigo-700 shadow-lg shadow-blue-500/25">
            ...
            <div className="flex-1 min-w-0 space-y-2">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {roles.map((r) => (
                  <span key={r} className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold border text-purple-300 bg-purple-500/15 border-purple-500/30 capitalize">
        ...
        <form onSubmit={(e) => handleSave(e, setUser)} noValidate className="space-y-6">
          ...
          {/* Admin-only: read-only system info */}
          <Section icon={Database} title="System Info" accent="text-purple-400"
            aside={<span className="text-xs text-slate-600">Read only</span>}>
            <div className="grid grid-cols-2 gap-5">
              <Field id="role" label="Role" icon={Key} value="Administrator" readOnly />
              <Field id="status" label="Account Status" icon={Shield} value={profile?.status ?? ""} readOnly />
              {profile?.emerald_mbr_id && (
                <Field id="mbr" label="Emerald MBR ID" icon={Hash} value={profile.emerald_mbr_id} readOnly />
              )}
              <Field id="joined" label="Member Since" icon={Database}
                value={profile?.created_at ? new Date(profile.created_at).toLocaleDateString("en-KE", { day: "numeric", month: "long", year: "numeric" }) : ""} readOnly />
            </div>
          </Section>
